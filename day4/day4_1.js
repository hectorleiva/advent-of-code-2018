...
var g = guardSleepTimes.reduce((acc, dateTimes) => {
  var idAccu = {};
  dateTimes.forEach((dateTime) => {
    if (!idAccu[dateTime.id]) idAccu[dateTime.id] = { size: 0, dateTimesCollection: [] };
    idAccu[dateTime.id] = {
      size: ++idAccu[dateTime.id].size,
      id: dateTime.id,
      dateTimesCollection: [
        ...idAccu[dateTime.id].dateTimesCollection,
        dateTime.time
      ]
    }
  });
...
var sleepestGuard = g.pop();
console.log('sleepest guard Id: ', sleepestGuard.id);

var asleepTimeCollection = sleepestGuard.dateTimesCollection.reduce((acc, minute) => {
  if (!acc[minute]) acc[minute] = { timesAsleep: 0 };
  acc[minute] = {
    timesAsleep: ++acc[minute].timesAsleep
  }

  return acc;
}, {});

console.log('asleepTimeCollection : ', asleepTimeCollection);